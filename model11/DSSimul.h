...
class MessageArg {
   public:
    enum { IntType = 'A', Int64Type, StringType, VectorIntType, EOFType };
    MessageArg(int64 q) {
        uint64 pq = (uint64)q;
    ...
    MessageArg(string const &s) { MessageArg(s.c_str()); }
...
class Message {
   public:
    ...
    Message(MessageArg const &a1) { append(a1); }
    ...
    bytevector body;
    string getString() {
        if (ptr < (int)body.size() &&
            body[ptr] == (byte)MessageArg::StringType) {
            string ret;
            ptr++;
    ...
    int getInt() {
        if (ptr + 4 < (int)body.size() &&
            body[ptr] == (byte)MessageArg::IntType) {
            unsigned ret = 0;
            ptr++;
            for (int i = 0; i <= 24; i += 8) {
                ret |= (body[ptr++] & 0xFF) << i;
    ...
    int64 getInt64() {
        if (ptr + 8 < (int)body.size() &&
            body[ptr] == (byte)MessageArg::Int64Type) {
            uint64 ret = 0;
            ptr++;
            for (int i = 0; i < 8; i++) {
...
class NetworkLayer {
   public:
    ...
    // Моделируется асинхронный режим. Сообщения посылаются процессу немедленно
    // и доставляются через время, указанное в свойствах связи. Процесс
    // принимает сообщения независимо от показания глобальных часов и от других
    // процессов.
    ...
    int send(int fromProcess, int toProcess, bytevector const &msg) {
        ...
        if (errorRate > 0 && distrib(rng) < errorRate)
   ...
   private:
    ...
    static void globalTimerExecutor(NetworkLayer *nl) {
        ...
        while (!nl->stopFlag) {
            ...
            nl->tick =
                chrono::duration_cast<chrono::milliseconds>(cl).count() / 1000;
...
class Process {
   public:
    ...
    set<int> neibs() { return networkLayer->neibs(node); }
    // Распределённый процесс может исполнять различные рабочие функции в
    // зависимости от пришедшего сообщения Каждая рабочая функция поддерживает
    // свой контекст исполнения, для разных функций он может быть разным
    ...
    // Если рабочая функция распознала сообщение, как предназначенное ей, она
    // возвращает true. Возможна ситуация, когда ни одна из рабочих функций не
    // обработает сообщение, тогда оно пропадает.
...
#include "contextes.h"
   private:
    ...
    static void workerThreadExecutor(Process *dp) {
        while (!dp->stopFlag) {
            // Поток обнаруживает появление сообщений в очереди принятых
            // сообщений и что-то делает в зависимости от самого сообщения
            if (dp->workerMessagesQueue.size() > 0 &&
                dp->networkLayer->tick >=
                    dp->workerMessagesQueue.peek().deliveryTime) {
...
class World {
   public:
    ...
    int assignWorkFunction(int node, string const &func) {
        if (node < 0 || node >= (int)processesList.size())
    ...
    bool parseConfig(string const &name) {
        ...
        while (!f.eof()) {
            ...
            if (sscanf(s, "bidirected %d", &bidirected) == 1)
                continue;
            else if (sscanf(s, "errorRate %lf", &errorRate) == 1)
                nl.setErrorRate(errorRate);
            else if (sscanf(s, "processes %d %d", &startprocess, &endprocess) ==
                     2) {
                for (int i = startprocess; i <= endprocess; i++)
                    createProcess(i);
            } else if (sscanf(s, "link from %d to %d latency %d", &from, &to,
                              &latency) == 3 ||
                       sscanf(s, "link from %d to %d", &from, &to) == 2) {
                nl.createLink(from, to, bidirected != 0, latency);
            } else if (sscanf(s, "link from %d to all latency %d", &from,
                              &latency) == 2 ||
                       sscanf(s, "link from %d to all", &from) == 1) {
                nl.addLinksToAll(from, bidirected != 0, latency);
            } else if (sscanf(s, "link from all to %d latency %d", &from,
                              &latency) == 2 ||
                       sscanf(s, "link from all to %d", &to) == 1) {
                nl.addLinksFromAll(from, bidirected != 0, latency);
            } else if (strcmp(s, "link from all to all") == 0 ||
                       sscanf(s, "link from all to all latency %d", &latency) ==
                           1) {
                nl.addLinksAllToAll(bidirected != 0, latency);
            } else if (sscanf(s, "setprocesses %d %d %s", &startprocess,
                              &endprocess, id) == 3) {
                for (int i = startprocess; i <= endprocess; i++)
                    assignWorkFunction(i, id);
            } else if (sscanf(s, "send from %d to %d %s %d", &from, &to, msg,
                              &arg) == 4) {
                nl.send(from, to, Message(msg, arg));
            } else if (sscanf(s, "send from %d to %d %s", &from, &to, msg) ==
                       3) {
